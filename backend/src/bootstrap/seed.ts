...
import { lastSevenDateKeys, parseHashtags, toDateKey } from '../utils/format';
import { ensureSeedImages } from './seed-images';
...
async function createUser(strapi: any, username: string, email: string, displayName: string, bio: string, avatarId: number | null) {
...
async function ensureFriendDogs(strapi: any, images: Map<string, string>): Promise<void> {
...
async function alignBruniCounts(strapi: any): Promise<void> {
...
export async function runSeed(strapi: any): Promise<void> {
  const images = await ensureSeedImages(strapi);
  ...
  // ---------- ข้อมูลอ้างอิง ----------
  const breedIds = new Map<string, number>();
  ...
  const kwan = await createUser(
    strapi,
    'browny.thegolden',
    'browny@instadog.app',
    'คุณขวัญ',
    'ทาสน้องบรูนี่ เต็มเวลา 🐾',
    bruniAvatar
  );
  const momoOwner = await createUser(
    strapi,
    'shiba.momo',
    'momo@instadog.app',
    'คุณมิ้นท์',
    'พาโมโม่ตะลุยคาเฟ่ทั่วกรุง',
    momoAvatar
  );
  ...
  const bruni = await strapi.db.query('api::dog.dog').create({
    data: {
      ...
      gender: 'male',
      birthDate: toDateKey(bruniBirth),
      weight: 28.5,
      vaccineStatus: 'วัคซีนครบถ้วน',
  ...
  for (const spec of POSTS) {
    ...
    const tagNames = parseHashtags(spec.caption);
    ...
    const post = await strapi.db.query('api::post.post').create({
      data: {
        ...
        mood: spec.mood,
        aiMoodSummary: (spec as any).aiMoodSummary ?? null,
        visibility: 'public',
        likeCount: spec.likeCount,
        commentCount: spec.commentCount,
        saveCount: Math.round(spec.likeCount / 12),
        hashtags: hashtagIds,
        exploreCategory: categoryIds.get(spec.category) ?? null,
        createdAt,
