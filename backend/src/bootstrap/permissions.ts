...
async function applyRole(strapi: any, roleType: string, allowed: string[]) {
  ...
  const existingByAction = new Map(existing.map((p: any) => [p.action, p]));

  for (const action of allowedSet) {
    const found: any = existingByAction.get(action);
    if (found) {
      if (!found.enabled) {
        await strapi.db
          .query('plugin::users-permissions.permission')
...
export async function applyPermissions(strapi: any): Promise<void> {
