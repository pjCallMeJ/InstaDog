...
function slugifyHandle(input) {
    const base = String(input !== null && input !== void 0 ? input : '')
        .toLowerCase()
        .normalize('NFKD')
        .replace(/[^a-z0-9._]+/g, '.')
        .replace(/\.{2,}/g, '.')
...
exports.default = strapi_1.factories.createCoreService('api::dog.dog', ({ strapi }) => ({
    /** กัน handle ชนกันโดยต่อท้ายด้วยตัวเลข */
    async buildUniqueHandle(source) {
