...
import { engine, strapi as strapi$1 } from '@strapi/data-transfer';
...
const isIgnoredContentType = (type)=>strapi$1.isIgnoredOfficialTransferType(type);
...
const logTransferFilterSummary = (opts)=>{
    ...
    if (only?.length) {
        ...
        if (omittedStages.length) {
            console.log(chalk.dim(`Stages not transferred (destination data preserved): ${omittedStages.join(', ')}.`));
        }
    }
...
const parseRestoreFromOptions = (opts, strapi1)=>{
    const entitiesOptions = {
        exclude: [
            ...
            ...strapi$1.getIgnoredOfficialTransferTypes(),
            ...opts.excludeContentTypes ?? []
        ],
        include: undefined
    };
