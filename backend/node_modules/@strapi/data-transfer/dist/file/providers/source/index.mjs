...
import { unknownPathToPosix, isFilePathInDirname, validateAssetMetadata, isPathEquivalent } from './utils.mjs';
   ...
   */ async bootstrap(diagnostics) {
    ...
    async validateStage(stage) {
            ...
            pipeline([
                ...
                new Parser({
                    filter (filePath, entry) {
                        ...
                        return isFilePathInDirname('assets/uploads', filePath) || isFilePathInDirname('assets/metadata', filePath);
                    },
                    async onReadEntry (entry) {
                        ...
                        try {
                            ...
                            validationError = new ProviderValidationError(`Asset metadata preflight failed for "${filename}": ${reason}`, {
                                error
                            });
                        } finally{
                            activeEntries -= 1;
        ...
        if (missingSidecars.length > 0) {
            throw new ProviderValidationError(`Asset metadata preflight failed: missing sidecar metadata for ${missingSidecars.map((filename)=>`"${filename}"`).join(', ')}. The destination was not modified; re-export and try again.`);
        }
        const metadata = new Map();
        for (const filename of uploads){
            try {
                metadata.set(filename, validateAssetMetadata(JSON.parse(sidecars.get(filename).toString()), filename));
            } catch (error) {
                const reason = error instanceof Error ? error.message : String(error);
                throw new ProviderValidationError(`Asset metadata preflight failed for "${filename}.json": ${reason}. The destination was not modified; re-export and try again.`, {
                    error
                });
            }
        }
...
async function loadAssetMetadata(sidecarPath) {
    ...
    return validateAssetMetadata(await _class_private_field_loose_base(this, _parseJSONFile)[_parseJSONFile](backupStream, sidecarPath), filename);
}
