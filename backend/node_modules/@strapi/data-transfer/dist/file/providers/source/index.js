...
var _metadata = /*#__PURE__*/ _class_private_field_loose_key("_metadata"), _assetMetadata = /*#__PURE__*/ _class_private_field_loose_key("_assetMetadata"), _diagnostics = /*#__PURE__*/ _class_private_field_loose_key("_diagnostics"), _reportInfo = /*#__PURE__*/ _class_private_field_loose_key("_reportInfo"), /**
   ...
   */ async bootstrap(diagnostics) {
    ...
    async validateStage(stage) {
            ...
            stream$1.pipeline([
                ...
                new tar.Parser({
                    filter (filePath, entry) {
                        ...
                        return utils.isFilePathInDirname('assets/uploads', filePath) || utils.isFilePathInDirname('assets/metadata', filePath);
                    },
                    async onReadEntry (entry) {
                        activeEntries += 1;
                        const normalizedPath = utils.unknownPathToPosix(entry.path);
                        const filename = path__default.default.basename(normalizedPath);
                        try {
                            if (utils.isFilePathInDirname('assets/uploads', normalizedPath)) {
                                uploads.add(filename);
                                entry.resume();
                                await promises.finished(entry);
                            } else {
                                const chunks = [];
                            ...
                            validationError = new providers.ProviderValidationError(`Asset metadata preflight failed for "${filename}": ${reason}`, {
                                error
                            });
                        } finally{
                            activeEntries -= 1;
        ...
        const missingSidecars = [
            ...uploads
        ].filter((filename)=>!sidecars.has(filename)).sort();
        if (missingSidecars.length > 0) {
            throw new providers.ProviderValidationError(`Asset metadata preflight failed: missing sidecar metadata for ${missingSidecars.map((filename)=>`"${filename}"`).join(', ')}. The destination was not modified; re-export and try again.`);
        }
        const metadata = new Map();
        for (const filename of uploads){
            try {
                metadata.set(filename, utils.validateAssetMetadata(JSON.parse(sidecars.get(filename).toString()), filename));
            } catch (error) {
                const reason = error instanceof Error ? error.message : String(error);
                throw new providers.ProviderValidationError(`Asset metadata preflight failed for "${filename}.json": ${reason}. The destination was not modified; re-export and try again.`, {
                    error
                });
            }
        }
        _class_private_field_loose_base(this, _assetMetadata)[_assetMetadata] = metadata;
    }
    ...
    createAssetsReadStream() {
        ...
        const assetMetadata = _class_private_field_loose_base(this, _assetMetadata)[_assetMetadata];
            ...
            new tar.Parser({
                ...
                async onReadEntry (entry) {
                    await runReadEntry(async ()=>{
                        ...
                        const metadata = assetMetadata?.get(file) ?? await loadAssetMetadata(`assets/metadata/${file}.json`).catch(()=>{
    ...
    constructor(options){
        ...
        Object.defineProperty(this, _assetMetadata, {
            writable: true,
            value: void 0
        });
...
async function loadAssetMetadata(sidecarPath) {
    const backupStream = _class_private_field_loose_base(this, _getBackupStream)[_getBackupStream]();
    const filename = path__default.default.posix.basename(sidecarPath).replace(/\.json$/, '');
    return utils.validateAssetMetadata(await _class_private_field_loose_base(this, _parseJSONFile)[_parseJSONFile](backupStream, sidecarPath), filename);
}
