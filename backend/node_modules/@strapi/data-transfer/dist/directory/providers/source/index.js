...
class LocalDirectorySourceProvider {
    ...
    async validateStage(stage) {
        ...
        if (!await fs__default.default.pathExists(uploadsDir)) {
            _class_private_field_loose_base(this, _assetMetadata)[_assetMetadata] = new Map();
            return;
        }
        const metadata = new Map();
        const names = (await fs__default.default.readdir(uploadsDir)).sort();
        for (const name of names){
            const uploadPath = path__default.default.join(uploadsDir, name);
            if (!(await fs__default.default.stat(uploadPath)).isFile()) {
                continue;
            }
            try {
                metadata.set(name, await _class_private_field_loose_base(this, _readAssetMetadata)[_readAssetMetadata](name));
            } catch (error) {
                const reason = error instanceof Error ? error.message : String(error);
                throw new providers.ProviderValidationError(`Asset metadata preflight failed for "${name}": ${reason}. The destination was not modified; re-export and try again.`, {
                    error
                });
            }
        }
...
async function pipeAssetsToStream(outStream, uploadsDir) {
    ...
    for (const name of names){
        ...
        if (stat.isFile()) {
            let metadata = _class_private_field_loose_base(this, _assetMetadata)[_assetMetadata]?.get(name);
            if (!metadata) {
                try {
...
async function readAssetMetadata(filename) {
    ...
    return utils.validateAssetMetadata(await fs__default.default.readJson(metadataPath), filename);
}
