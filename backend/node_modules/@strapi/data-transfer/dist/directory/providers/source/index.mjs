...
import { unknownPathToPosix, validateAssetMetadata } from '../../../file/providers/source/utils.mjs';
...
class LocalDirectorySourceProvider {
    ...
    async validateStage(stage) {
        ...
        for (const name of names){
            ...
            if (!(await fs__default.stat(uploadPath)).isFile()) {
                continue;
            }
            try {
                ...
                throw new ProviderValidationError(`Asset metadata preflight failed for "${name}": ${reason}. The destination was not modified; re-export and try again.`, {
                    error
                });
            }
        }
...
async function readAssetMetadata(filename) {
    ...
    return validateAssetMetadata(await fs__default.readJson(metadataPath), filename);
}
