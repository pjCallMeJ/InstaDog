 ...
 */ const mediaIdSchema = utils.z.number().int().min(1).describe('Numeric media asset id (e.g. 42). Media files are not documents — they have no documentId and no draft/published versions.');
const folderIdSchema = utils.z.number().int().min(1).describe('Numeric media folder id (e.g. 3). Folders are not documents — they use numeric ids.');
const pageSchema = utils.z.number().int().min(1).optional().describe('Page number (1-indexed, default: 1).');
const pageSizeSchema = utils.z.number().int().min(1).max(100).optional().describe('Items per page (default: 25, max: 100).');
/**
 ...
 */ const sortSchema = utils.z.enum(constants.ALLOWED_SORT_STRINGS).optional().describe(`Sort expression. One of: ${constants.ALLOWED_SORT_STRINGS.join(', ')}. Defaults to "createdAt:DESC".`);
const mediaListAssetsInputSchema = utils.z.object({
    ...
    mime: utils.z.string().min(1).optional().describe('Filter by mime type prefix or exact value (e.g. "image", "image/png", "application/pdf").'),
    name: utils.z.string().min(1).optional().describe('Case-insensitive substring search on the asset name.'),
    page: pageSchema,
    pageSize: pageSizeSchema,
    sort: sortSchema
});
const mediaGetAssetInputSchema = utils.z.object({
    id: mediaIdSchema
});
utils.z.object({});
 ...
 */ const mediaUpdateAssetInputSchema = utils.z.object({
    id: mediaIdSchema,
    name: utils.z.string().min(1).optional().describe('New asset name as shown in the Media Library. Renames the entry only — the stored file and its URL are unchanged.'),
    alternativeText: utils.z.string().nullable().optional().describe('Alt text used by the frontend for accessibility. Pass null to clear it; the field then reads back as an empty string.'),
    caption: utils.z.string().nullable().optional().describe('Caption shown alongside the asset. Pass null to clear it; the field then reads back as an empty string.')
}, {
    error (issue) {
 ...
 */ const folderNameSchema = utils.z.string().min(1).regex(/^[^/]+$/, 'Folder name cannot contain slashes.').regex(/^(?! ).+(?<! )$/, 'Folder name cannot start or end with a whitespace.').describe('Folder name as shown in the Media Library. Cannot contain slashes or start/end with a space, and must be unique among its siblings.');
/**
...
const mediaCreateFolderInputSchema = utils.z.object({
 ...
 */ const mediaRenameFolderInputSchema = utils.z.object({
    id: folderIdSchema,
    name: folderNameSchema
}, {
    error (issue) {
 ...
 */ const mediaMoveFolderInputSchema = utils.z.object({
    id: folderIdSchema,
 ...
 */ const mediaDeleteFolderInputSchema = utils.z.object({
    ids: utils.z.array(folderIdSchema).min(1).max(100).describe('Numeric ids of the folders to delete (1-100). FOLDER ids only — asset ids are a separate namespace of integers and are rejected here; use media_delete_assets for assets.'),
    dryRun: utils.z.boolean().optional().describe('When true (the default), nothing is deleted and the tool only reports how many folders and files WOULD be removed. Pass false to actually perform the irreversible deletion.')
}).strict();
/**
 ...
 */ const mediaDeleteAssetsInputSchema = utils.z.object({
    ids: utils.z.array(mediaIdSchema).min(1).max(100).describe('Numeric ids of the assets to delete (1-100). ASSET ids only, taken from media_list_assets or media_get_asset — never from media_list_folders. Folder ids are numbered separately and the same number often names both an asset and a folder, so a folder id here deletes the asset sharing that number; use media_delete_folder for folders.'),
    dryRun: utils.z.boolean().optional().describe('When true (the default), NOTHING is deleted and the tool only reports which assets WOULD be permanently removed. Pass false to actually perform the irreversible deletion.')
}, {
    error (issue) {
        if (issue.code === 'unrecognized_keys' && issue.keys.some((key)=>key === 'id' || key === 'fileIds')) {
...
exports.folderIdSchema = folderIdSchema;
exports.folderNameSchema = folderNameSchema;
exports.mediaCreateFolderInputSchema = mediaCreateFolderInputSchema;
exports.mediaDeleteAssetsInputSchema = mediaDeleteAssetsInputSchema;
exports.mediaDeleteFolderInputSchema = mediaDeleteFolderInputSchema;
exports.mediaGetAssetInputSchema = mediaGetAssetInputSchema;
exports.mediaIdSchema = mediaIdSchema;
exports.mediaListAssetsInputSchema = mediaListAssetsInputSchema;
exports.mediaMoveAssetsInputSchema = mediaMoveAssetsInputSchema;
exports.mediaMoveFolderInputSchema = mediaMoveFolderInputSchema;
exports.mediaRenameFolderInputSchema = mediaRenameFolderInputSchema;
exports.mediaUpdateAssetInputSchema = mediaUpdateAssetInputSchema;
