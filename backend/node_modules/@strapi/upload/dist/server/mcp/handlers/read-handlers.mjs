...
import { sanitizeMediaAsset, sanitizeMediaFolderTree } from '../sanitizers/sanitize-media.mjs';
import { MCP_NOT_FOUND_ASSET } from './constants.mjs';
 ...
 */ const createMediaGetAssetHandler = (strapi, context)=>{
    ...
    return async ({ args })=>{
        ...
        let asset;
        try {
            ({ file: asset } = await findEntityAndCheckPermissions(context.userAbility, ACTIONS.read, FILE_MODEL_UID, id, strapi));
        } catch (error) {
 ...
 */ const createMediaListFoldersHandler = (strapi, context)=>{
    ...
    return async ()=>{
        ...
        return ok({
            data: sanitizeMediaFolderTree(structure)
        });
    };
};

export { createMediaGetAssetHandler, createMediaListAssetsHandler, createMediaListFoldersHandler };
//# sourceMappingURL=read-handlers.mjs.map
