...
import { assertAmbientInstance, getFolderService } from '../ambient-instance.mjs';
import { FOLDER_MODEL_UID, FILE_MODEL_UID, ACTIONS } from '../../constants.mjs';
import { isFolderOrChild } from '../../controllers/utils/folders.mjs';
import { assertMediaPermission } from '../permissions.mjs';
import { sanitizeMediaFolder } from '../sanitizers/sanitize-media.mjs';
import { MCP_NOT_FOUND_FOLDER, MCP_DELETE_FOLDER_UNRESOLVED_IDS, MCP_PARENT_FOLDER_NOT_FOUND, MCP_FOLDER_NAME_TAKEN, MCP_FOLDER_MOVE_INTO_SELF } from './constants.mjs';
 ...
 */ const findFolderById = async (strapi, id, { withParent = false } = {})=>strapi.db.query(FOLDER_MODEL_UID).findOne({
        select: [
            'id',
            'name',
            'path'
 ...
 */ const assertParentExists = async (strapi, parent)=>{
    ...
    const exists = await getFolderService(strapi).exists({
        id: parent
    });
    if (!exists) {
 ...
 */ const countCascade = async (strapi, paths)=>{
    ...
    const [totalFolderNumber, totalFileNumber] = await Promise.all([
        strapi.db.query(FOLDER_MODEL_UID).count({
            where: pathPredicate('path')
        }),
        strapi.db.query(FILE_MODEL_UID).count({
 ...
 */ const createMediaCreateFolderHandler = (strapi, context)=>{
    ...
    return async ({ args })=>{
        ...
        const created = await getFolderService(strapi).create({
            name,
            parent
        }, {
            user: context.user
 ...
 */ const createMediaRenameFolderHandler = (strapi, context)=>{
    ...
    return async ({ args })=>{
        ...
        const renamed = await getFolderService(strapi).update(id, {
            name
        }, {
            user: context.user
        });
 ...
 */ const createMediaMoveFolderHandler = (strapi, context)=>{
    ...
    return async ({ args })=>{
        ...
        const moved = await getFolderService(strapi).update(id, {
            name: folder.name,
            parent
        }, {
            user: context.user
 ...
 */ const createMediaDeleteFolderHandler = (strapi, context)=>{
    ...
    return async ({ args })=>{
        ...
        assertMediaPermission(strapi, context, ACTIONS.update, FOLDER_MODEL_UID);
        const matched = await strapi.db.query(FOLDER_MODEL_UID).findMany({
            select: [
                'id',
                'name',
                'path'
     ...
     */ const matchedIds = new Set(matched.map((folder)=>folder.id));
        ...
        const { totalFolderNumber, totalFileNumber } = await getFolderService(strapi).deleteByIds(matched.map((folder)=>folder.id));
        return ok({
            dryRun: false,
            folders,
            totalFolderNumber,
...
export { createMediaCreateFolderHandler, createMediaDeleteFolderHandler, createMediaMoveFolderHandler, createMediaRenameFolderHandler };
//# sourceMappingURL=folder-handlers.mjs.map
