...
var trimGrantSessionResponse = require('../trim-grant-session-response.js');
...
function requireOauthConnect() {
    ...
    const { trimGrantSessionResponse: trimGrantSessionResponse$1 } = trimGrantSessionResponse.__require();
    ...
    const redirectWithSessionResponse = (ctx, callbackUrl, provider, tokenResponse)=>{
        const grantResponse = oauth2$1.tokensToQueryPayload(provider, tokenResponse);
        ctx.session.grant = {
            response: trimGrantSessionResponse$1(grantResponse, provider.name)
        };
