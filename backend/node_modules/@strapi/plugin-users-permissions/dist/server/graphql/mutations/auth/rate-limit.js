...
function requireRateLimit() {
    ...
    const enqueueContextOperation = (koaContext, operation)=>{
        ...
        contextQueues.set(koaContext, result.catch(()=>undefined));
        return result;
    };
    const createRateLimitRunner = (strapi, routeSuffix)=>{
        const rateLimit = strapi.middleware(RATE_LIMIT_MIDDLEWARE)({}, {
            strapi
        });
        const routePath = getRateLimitPath(strapi, routeSuffix);
        return (koaContext, { body, params }, controller)=>enqueueContextOperation(koaContext, async ()=>{
