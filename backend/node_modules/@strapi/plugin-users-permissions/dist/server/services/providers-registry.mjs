    ...
    const initProviders = ({ baseURL })=>({
            ...
            github: {
                ...
                async authCallback ({ accessToken }) {
                    ...
                    if (userBody.email) {
                        const verifiedPublicEmail = Array.isArray(emailBody) ? emailBody.find((entry)=>entry.email === userBody.email && entry.verified === true) : null;
                    ...
                    const primaryEmail = Array.isArray(emailBody) ? emailBody.find((email)=>email.primary === true) : null;
            ...
            patreon: {
                ...
                async authCallback ({ accessToken }) {
                    const { body } = await bearerGet('https://www.patreon.com/api/oauth2/v2/identity?fields[user]=full_name,email,is_email_verified', accessToken);
