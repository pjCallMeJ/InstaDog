...
const BlocksEditor = /*#__PURE__*/ React__namespace.forwardRef(({ disabled = false, name, onChange, value, error, livePreviewSync = false, blockIndex, ...contentProps }, forwardedRef)=>{
    ...
    React__namespace.useEffect(()=>{
        ...
        const raf = requestAnimationFrame(()=>{
            try {
                const blockNode = slate.Node.get(editor, [
                    targetIndex
                ]);
                const domEl = slateReact.ReactEditor.toDOMNode(editor, blockNode);
                ...
                const scrollContainer = editorDomEl?.parentElement;
                if (scrollContainer) {
                    const blockRect = domEl.getBoundingClientRect();
                    const containerRect = scrollContainer.getBoundingClientRect();
                    // 24px = BlocksContent paddingTop={6} (design-system 4px scale)
                    scrollContainer.scrollTop = blockRect.top - containerRect.top + scrollContainer.scrollTop - 24;
                } else {
