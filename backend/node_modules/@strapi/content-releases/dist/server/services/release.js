    ...
    return {
        async create (releaseData, { user }) {
            ...
            await utils.emitAudit({
                strapi
            }, constants.AUDITED_EVENTS.RELEASE_CREATE, {
        ...
        async update (id, releaseData, { user }) {
            ...
            if (updatedRelease) {
                const changes = auditLogs.getReleaseChanges(release, updatedRelease);
                if (Object.keys(changes).length > 0) {
                    await utils.emitAudit({
                        strapi
                    }, constants.AUDITED_EVENTS.RELEASE_UPDATE, {
        ...
        async delete (releaseId) {
            ...
            await utils.emitAudit({
                strapi
            }, constants.AUDITED_EVENTS.RELEASE_DELETE, {
                releaseId: release.id,
                name: release.name
            });
            return release;
        ...
        async publish (releaseId) {
            const { release, error, lockedRelease } = await strapi.db.transaction(async ({ trx })=>{
                /**
         ...
         */ const lockedRelease = await strapi.db?.queryBuilder(constants.RELEASE_MODEL_UID).where({
            ...
            if (error !== null) {
                await utils.emitAudit({
                    strapi
                }, constants.AUDITED_EVENTS.RELEASE_TRIGGER, {
            ...
            try {
                ...
                countsError = err instanceof Error ? err : new Error(String(err), {
                    cause: err
                });
                strapi.log.error(`Failed to count the entries for the release.trigger entry of release ${releaseId}`, {
                    error: countsError
                });
            }
            await utils.emitAudit({
