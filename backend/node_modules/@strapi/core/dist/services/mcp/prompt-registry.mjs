...
import { createMcpCapabilityHandlerContext } from './utils/createMcpCapabilityHandlerContext.mjs';
import { toSdkPromptResult } from './utils/toSdkMcpCapabilityResult.mjs';
            ...
            return createSafeCapabilityRegistration({
                ...
                registerWithSdk (safeHandler) {
                    if (argsSchema === undefined) {
                        ...
                        const sdkHandler = wrapCapabilityHandlerForMetrics(strapi, 'prompt', name, definition.telemetry, async (context)=>toSdkPromptResult(await handler(context)));
                    ...
                    const handler = safeHandler;
                    const sdkHandler = wrapCapabilityHandlerForMetrics(strapi, 'prompt', name, definition.telemetry, async (args, context)=>toSdkPromptResult(await handler(args, context)));
                    return mcpServer.registerPrompt(name, {
                        title,
                        description,
                        argsSchema
...
export { McpPromptRegistry, makeMcpPromptDefinition };
